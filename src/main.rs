...
fn main() -> io::Result<()> {
  ...
  //sha256sum <(echo gnostr-legit)
	...
	let event = if cfg!(target_os = "windows") {
		Command::new("cmd")
                .args(["/C", "gnostr --sec $(gnostr-sha256 $(gnostr-weeble || echo)) --tag weeble $(gnostr-weeble || echo weeble) --tag wobble $(gnostr-wobble || echo wobble) --tag blockheight $(gnostr-blockheight || echo blockheight) --content \"$(gnostr-git diff HEAD~1 || gnostr-git diff)\" "])
                .output()
                .expect("failed to execute process")
	} else if cfg!(target_os = "macos") {
		Command::new("sh")
                .args(["-c", "gnostr --sec $(gnostr-sha256 $(gnostr-weeble || echo)) --tag weeble $(gnostr-weeble || echo weeble) --tag wobble $(gnostr-wobble || echo wobble) --tag blockheight $(gnostr-blockheight || echo blockheight) --content \"$(gnostr-git show HEAD)\" "])
                .output()
                .expect("failed to execute process")
	} else if cfg!(target_os = "linux") {
		Command::new("sh")
